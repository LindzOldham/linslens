...
###
    ...
    def GetPhotometry(self,K=False):
        if bands[self.name] == 'F555W':
            vband, bband = MassB2, MassB2toB
        elif bands[self.name] == 'F606W':
            vband, bband = MassB1, MassB1toB
    ...
    def GetPhotometryAtAge(self,K=False,age='4.000'):
        if bands[self.name] == 'F555W':
            vband, bband = MassB2, MassB2toB
        elif bands[self.name] == 'F606W':
            vband, bband = MassB1, MassB1toB
    ...
    def UncertaintiesFromPDF(self,makecat=False):
        magPDF,magrestPDF,muPDF,murestPDF,RePDF,LumPDF = np.array(self.magPDF),np.array(self.magrestPDF),np.array(self.muPDF),np.array(self.murestPDF),np.array(self.RePDF),np.array(self.LumPDF)
        keys = ['magPDF','magrestPDF','muPDF','murestPDF','RePDF','LumPDF']
        vals = [magPDF,magrestPDF,muPDF,murestPDF,RePDF,LumPDF]
        l,d,u=[],[],[]
        f1,f2,f3 = np.zeros((len(keys),3)),np.zeros((len(keys),3)),np.zeros((len(keys),3))
        for i in range(len(vals)):
            key = keys[i]
            f = vals[i]
            if f.shape[1] ==3:
                V,I,B =  map(lambda v: (v[1]-v[0],v[1],v[2]-v[1]),zip(*np.percentile(f,[16,50,84],axis=0)))
            else:
                V,I =  map(lambda v: (v[1]-v[0],v[1],v[2]-v[1]),zip(*np.percentile(f,[16,50,84],axis=0)))
                B = np.zeros(3)
            d.append((key,[V[1],I[1],B[1]]))
            l.append((key,[V[0],I[0],B[0]]))
            u.append((key,[V[2],I[2],B[2]]))
            f1[i],f2[i],f3[i] = [V[1],I[1],B[1]],[V[0],I[0],B[0]], [V[2],I[2],B[2]]
