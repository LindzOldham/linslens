...
def MakeTab(model,kmodel,name):
    ...
    pl.imshow((imgs[0]-mods[0])/sigs[0],interpolation='nearest',origin='lower',cmap='gray_r',vmin=-5,vmax=5)
    pl.subplot(143)
    ###
    ...
    pl.gca().xaxis.set_ticks([])
    pl.gca().yaxis.set_ticks([])
    pl.imshow((imgs[1]-mods[1])/sigs[1],interpolation='nearest',origin='lower',cmap='gray_r',vmin=-5,vmax=5)
    kmodel.Initialise()
    ...
    pl.gca().yaxis.set_ticks([])
    if name == 'J1619':
        pl.imshow((img-mod)/sig,interpolation='nearest',origin='lower',cmap='gray_r',vmin=-3,vmax=3)
    else:
        pl.imshow((img-mod)/sig,interpolation='nearest',origin='lower',cmap='gray_r',vmin=-2,vmax=2)
    pl.subplot(141)
    ...
    pl.gca().xaxis.set_ticks([])
    pl.gca().yaxis.set_ticks([])
    #pl.savefig('/data/ljo31/Lens/TeXstuff/paper/'+str(model.name)+'resid.pdf')
    pl.show()
