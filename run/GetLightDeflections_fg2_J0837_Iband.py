...
def Deflect(model,kmodel,name):
    ...
    if model.galno == 2.:
        ...
        ii = np.where(model.fits[1][0:2]!=0.)
        if len(ii[0])<len(model.gals):
            print 'one of the components has no flux!'
            print 'component ', '%.2f'%np.where(model.fits[1][0:2]==0), 'has no flux so we are removing it'
            model.galno = 1.
            ...
            mk1,mk2 = gali1.getMag(model.fits[1][0],0), gali2.getMag(model.fits[1][1],0)
    ...
    outFile = '/data/ljo31b/EELs/galsub/light_deflections_J0837_Iband.npy'
    deflections = [V,I,K]
    f = open(outFile,'wb')
