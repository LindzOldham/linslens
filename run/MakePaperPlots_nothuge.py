...
def MakeTab(model,kmodel,name):
    ...
    pl.imshow((imgs[0]-mods[0])/sigs[0],interpolation='nearest',origin='lower',cmap='Greys',vmin=-5,vmax=5)
    pl.subplot(143)
    ###
    ...
    pl.gca().xaxis.set_ticks([])
    pl.gca().yaxis.set_ticks([])
    pl.imshow((imgs[1]-mods[1])/sigs[1],interpolation='nearest',origin='lower',cmap='Greys',vmin=-5,vmax=5)
    kmodel.Initialise()
    ...
    pl.gca().yaxis.set_ticks([])
    if name == 'J1619':
        pl.imshow((img-mod)/sig,interpolation='nearest',origin='lower',cmap='Greys',vmin=-3,vmax=3)
    else:
        pl.imshow((img-mod)/sig,interpolation='nearest',origin='lower',cmap='Greys',vmin=-2,vmax=2)
    pl.subplot(141)
    ...
    pl.gca().xaxis.set_ticks([])
    pl.gca().yaxis.set_ticks([])
    pl.savefig('/data/ljo31/Lens/TeXstuff/paper/'+str(model.name)+'resid2.pdf')
    pl.suptitle('nothuge')
    pl.show()
    
    

dir = '/data/ljo31/Lens/LensModels/'
sz = np.load('/data/ljo31/Lens/LensParams/SourceRedshiftsUpdated.npy')[()]
names = sz.keys()
names.sort()
shifts = [['J0837',[1.3,2.9]],['J0901',[0,3]],['J0913',[0,0]],['J1125',[0.9,2.1]],['J1144',[-0.8,4.55]],['J1218',[-2.2,-5.7]],['J1323',[1.0,1.1]],['J1347',[3.7,0]],['J1446',[0,0]],['J1605',[-3.5,0.1]],['J1606',[-4.8,7.6]],['J1619',[3.8,0]],['J2228',[-2.4,-3.6]]]
...
for name in names[3:]:
    ...
    try:
        if name == 'J1144':
            result = np.load(dir+name+'_212_allparams')
            kresult = np.load(dir[:-8]+name+'_Kp_212')
        elif name == 'J1605':
            result = np.load(dir+name+'_212_final')
            kresult = np.load(dir[:-8]+name+'_Kp_212')
        else:
        ...
        if name == 'J1347':
            ...
            kresult = np.load(dir+name+'_Kp_112')
        elif name == 'J1619':
            result = np.load(dir+name+'_212')
