...
for ii in range(len(names)):
    name = names[ii]
    re, mag =  np.load('/data/ljo31/Lens/Analysis/ReMagPDFs_lensgals_'+name+'.npy')
    zs = lz[name][0]
    da = astCalc.da(zs)
    ...
    elif name in ['J1144','J1606','J1125']:
        xbins=np.linspace(min(x)-1.5,max(x)+0.5,50)
        ybins=np.linspace(min(y)-0.1,max(y)+0.1,50)
    elif name == 'J1323':
    ...
    print sigmas
pl.close('all')
np.save('/data/ljo31/Lens/LensParams/RelogI_lensgals_covariances.npy',sigmas)
