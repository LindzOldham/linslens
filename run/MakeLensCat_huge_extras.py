...
def MakeTab(model):
    ...
    np.save('/data/ljo31/Lens/PDFs/211_huger__'+str(model.name), [np.array(model.muPDF),np.array(model.magPDF),np.array(model.RePDF)])
    return med,lo,hi
...
tbhdu.writeto('/data/ljo31/Lens/LensParams/Phot_1src_huge_extras.fits',clobber=True)
'''
