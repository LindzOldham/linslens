...
kphot = np.load('/data/ljo31/Lens/LensParams/KeckPhot_1src_huge_new_dict.npy')[()] 
names = phot['name']
...
table = np.load('/data/ljo31/Lens/LensParams/Structure_2src_huge_new.npy')[()]
m,l,u = table
...
for ii in range(len(names)):
    ...
    print name, '&', lz[name], '&', sz[name], '& $', '%.2f'%(0.05*m[name]['Lens 1 b']), '\pm', '%.2f'%(0.05*l[name]['Lens 1 b']), '$ & $', '%.2f'%m[name]['Lens 1 eta'], '\pm', '%.2f'%l[name]['Lens 1 eta'], '$ & $', '%.2f'%m[name]['Lens 1 q'], '\pm', '%.2f'%l[name]['Lens 1 q'], '$ & $','%.2f'%m[name]['Lens 1 pa'], '\pm', '%.2f'%l[name]['Lens 1 pa'], '$ & $', '%.2f'%m[name]['extShear'], '\pm', '%.2f'%l[name]['extShear'], '$ & $', '%.2f'%m[name]['extShear PA'], '\pm', '%.2f'%l[name]['extShear PA'], r'$ \\'


phot = py.open('/data/ljo31/Lens/LensParams/Phot_1src_huge_new.fits')[1].data
kphot = np.load('/data/ljo31/Lens/LensParams/KeckPhot_1src_huge_new_dict.npy')[()] # update
phot2 = py.open('/data/ljo31/Lens/LensParams/Phot_2src_huge_new.fits')[1].data
kphot2 = np.load('/data/ljo31/Lens/LensParams/KeckPhot_2src_new_dict.npy')[()] # update
table = np.load('/data/ljo31/Lens/LensParams/Structure_1src_huge_new.npy')[()]
m,l,u = table
table2 = np.load('/data/ljo31/Lens/LensParams/Structure_2src_huge_new.npy')[()]
m2,l2,u2 = table2
for i in range(3):
    print '%%%'
