...
name = 'J1619'
masses = np.load('/data/ljo31/Lens/Analysis/ReMassTrace_200_asigo_j1619_lensgals.npy').T[:50]
res = np.load('/data/ljo31/Lens/Analysis/ReMagPDFs_lensgals_'+str(name)+'.npy')[0].T[0][:50]
sigmas = np.zeros(4)
pl.title(name)
jj = clip2d(res,masses)
x,y = np.log10(res[jj]), masses[jj]
xbins=np.linspace(min(x)-0.5,max(x)+0.5,100)
ybins=np.linspace(min(y)-0.1,max(y)+0.1,100)
sigmas = ellipse(x,y,xbins,ybins)
    
np.save('/data/ljo31/Lens/LensParams/ReMass_covariances_lensgal_j1619.npy',sigmas)
