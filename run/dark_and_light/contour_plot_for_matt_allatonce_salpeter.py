...
def plot(name):    
    ...
    pl.contour(H.T,[lvl95,lvl68],colors=colours[ii],extent=(xbins[0],xbins[-1],ybins[0],ybins[-1]))

    Mstar = salp_m[name==names]
    Mstar = 10**Mstar / (1e11)
    print Mstar, ml_dpl.mean(), 10**salp_m[name==names] / (1e11)
    pl.xlabel('dark halo inner slope',fontsize=30)
    pl.ylabel('stellar mass \n relative to a Salpeter IMF',fontsize=30)
    pl.axvline(1,color='k')
    pl.axhline(0,color='k')
    if name == 'J0837':
        pl.text(0.81,1,'NFW',rotation=90,fontsize=28)
        pl.text(0.5,-0.19,'SALPETER',fontsize=28,horizontalalignment='center')


    pl.savefig('/data/ljo31/public_html/Lens/dark_and_light/compare_profiles_new/contour_plot_all_salpeter.pdf')
