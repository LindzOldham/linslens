...
def MakeModel(name,result,oldResult=None):
    ...
    GAMMA = pymc.Uniform('Lens 1 gamma',0.005,2.75,value=1.)#dic['Lens 1 eta'][a1,a2])
    RS = pymc.Uniform('Lens 1 rs',0.05,1000.,value=100.)#dic['Lens 1 b'][a1,a2])
    ...
    S = myEmcee.PTEmcee(pars+[likelihood],cov=cov,nwalkers=100,ntemps=1)#3)
    #if oldResult is not None:
    #    S.p0 = trace[-1]
    S.sample(5)

    print 'done emcee'
    outFile = '/data/ljo31b/EELs/galsub/emceeruns/test'
