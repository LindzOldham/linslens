...
def MakeModel(name,result,Npnts=2):
    ...
    def doFit(p=None,updateReg=False,checkImgs=True,doReg=True):
        global reg
        reg=10.
        lp = 0.
        ...
        for ii in range(len(ifltms)-1):
            ...
            srcs[ii].update(xl,yl,doReg=doReg)
    ...
    # check initial model
    doFit(updateReg=False,checkImgs=False)
    doFit(updateReg=False,checkImgs=True)
...
pix_mask = py.open('/data/ljo31b/EELs/galsub/masks/'+name+'.fits')[0].data.copy()[my-45:my+45,mx-45:mx+45]
pix_mask = pix_mask==1
...
#sig1 = py.open('/data/ljo31b/EELs/galsub/images/'+name+'_maxlnL_sig.fits')[1].data[my-45:my+45,mx-45:mx+45] 
...
result = np.load('/data/ljo31/Lens/J0837/basickest_NEWIMS_BIGIMS_2')
lp,trace,dic,_ = result
