...
def MakeModel(name,result,oldResult=None):
    ...
    RS = pymc.Uniform('Lens 1 rs',0.05,450.,40) # 100 kpc 
    ...
    #print Q.value
    lens1 = MassModels.DPL('Lens 1',{'x':X,'y':Y,'b':B,'eta1':GAMMA,'eta2':3-GAMMA,'rs':RS,'q':Q,'pa':PA})
    shear = MassModels.ExtShear('shear',{'x':X,'y':Y,'b':SH,'pa':SHPA})
    lenses = [lens1,shear]
    pars = [X,Y,B,Q,GAMMA,RS,PA,SH,SHPA]
    cov = [0.5,0.5,5.,0.1,0.5,30.,5.,0.05,5.]
    ...
    print 'logMstar', log_Mstar
    Mstar_start = (10**log_Mstar) * 1e-12 #*0.1
    ...
    S = myEmcee.PTEmcee(pars+[likelihood],cov=cov,nthreads=30,nwalkers=100,ntemps=6)#3)
    #if oldResult is not None:
    #    S.p0 = trace[-1]
    S.sample(500)

    print 'done emcee'
    outFile = '/data/ljo31b/EELs/galsub/emceeruns/'+name+'_parametric_DPL_VI_0'
