...
def MakeModel(name,result,oldResult=None):
    ...
    B = pymc.Uniform('Lens 1 b',0.05,100.,value=dic['Lens 1 b'][a1,a2]/4.)
    ...
    # wider points
    cov_u = [1.,1.,2.,0.2,0.4,5.,0.05,5.]
    cov_n = [0.,0.,0.,0.,0.,0.,0.,0.]
    #pars = [X,Y,B,Q,ETA,SH,SHPA]
    #cov_u = [0.,0.,2.,0.,0.5,0,0]
    #cov_n = [0.5,0.5,0.,0.1,0.,0.05,5.]
    ...
    Mstar_start = (10**log_Mstar) *1e-12
    print Mstar_start
    Mstar = pymc.Uniform('stellar mass',0.001,5.,Mstar_start)
    pars += [Mstar]
    cov_u += [Mstar_start*0.5]
    cov_n += [0]
    ...
    if oldResult is not None:
        ...
        for i in range(len(pars)):
            ...
            #    pars[i].value = trace[a1,0,a3,i]
            #elif i>3.:
            #    pars[i].value = trace[a1,0,a3,i-1]
            #else:
            #    print 'q'
    ...
    print 'done emcee'
    outFile = '/data/ljo31b/EELs/galsub/emceeruns/'+name+'_parametric_VI_3'
    ...
    for i in range(len(imgs)):
        ...
        models.append(model)
        #py.writeto('VI_2_'+colours[i]+'.fits',(image-model)/sigma,clobber=True)
        #py.writeto('VI_2_img_old_'+colours[i]+'.fits',image,clobber=True)
        #py.writeto('VI_2_mod_'+colours[i]+'.fits',model,clobber=True)
        
        #pl.figure()
        #pl.imshow((image-model)/sigma,interpolation='nearest',origin='lower',vmin=-5,vmax=5)
        #pl.colorbar()
...
'''if name == 'J1144':
    mask2 = py.open('/data/ljo31b/EELs/galsub/masks/J1144newmodel_I_maxlnL_mask.fits')[0].data
    mask+=mask2
    pl.figure()
    pl.imshow(mask)
    pl.show()'''
...
# 10: using old images, starting from VI_7
# 11: using new images, starting from I_4, using an extra maskq
#print f
#oldResult = np.load(f)
if name == 'J1125':
