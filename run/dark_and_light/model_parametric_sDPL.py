...
def MakeModel(name,result,oldResult=None):
    ...
    RS = pymc.Uniform('Lens 1 rs',0.05,450.) # 100 kpc 
    ...
    lens1 = MassModels.sGNFW('Lens 1',{'x':X,'y':Y,'b':B,'eta':GAMMA,'rs':RS})
    shear = MassModels.ExtShear('shear',{'x':X,'y':Y,'b':SH,'pa':SHPA})
    lenses = [lens1,shear]
    pars = [X,Y,B,GAMMA,RS,SH,SHPA]
    cov = [0.5,0.5,5.,0.5,100.,0.05,5.]
    ...
    S = myEmcee.PTEmcee(pars+[likelihood],cov=cov,nthreads=20,nwalkers=100,ntemps=6)#3)
    #if oldResult is not None:
    #    S.p0 = trace[-1]
    S.sample(500)

    print 'done emcee'
    outFile = '/data/ljo31b/EELs/galsub/emceeruns/'+name+'_parametric_sDPL_4'
