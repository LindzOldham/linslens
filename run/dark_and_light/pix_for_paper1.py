...
def showRes(x,y,src,psf,img,sig,mask,iflt,vflt,cmat,reg,niter,npix,vmx,band):
    ...
    pl.subplot(131)
    pl.figtext(0.05,0.8,name)
    pl.imshow(oimg,origin='lower',interpolation='nearest',extent=ext,vmin=0,vmax=vmx,cmap='jet',aspect='auto')
    ...
    pl.gca().yaxis.set_major_locator(pl.NullLocator())
    pl.subplot(133)
    pl.imshow(osrc,origin='lower',interpolation='nearest',extent=ext2,vmin=0,vmax=vmx,cmap='jet',aspect='auto')
    pl.gca().xaxis.set_major_locator(pl.NullLocator())
    pl.gca().yaxis.set_major_locator(pl.NullLocator())
...
def MakeModel(name,result,Npnts=1):
    ...
    def doFit(p=None):
        ...
        for l in lenses:
            l.setPars()
        regs = []
        for ii in range(len(ifltms)): ### IMPORTANT - JUST DO THE V BAND FOR NOW!!!
