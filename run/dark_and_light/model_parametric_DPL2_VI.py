...
def MakeModel(name,result,oldResult=None):
    ...
    # wider points
    cov_u = [1.,1.,2.,0.1,0.4,400,5.,0.05,5.]
    cov_n = [0.,0.,0.,0.,0.,0.,0.,0.,0.]
    ...
    print 'done emcee'
    outFile = '/data/ljo31b/EELs/galsub/emceeruns/'+name+'_parametric_DPL__VI_5'
...
mask = mask[my-XX:my+XX,mx-XX:mx+XX]
...
if name == 'J0837':
    V,I,_ = np.load('/data/ljo31b/EELs/galsub/deflections/light_deflections_'+name+'_Iband.npy')
...
oldFiles = glob.glob('/data/ljo31b/EELs/galsub/emceeruns/'+name+'_parametric_DPL_*')
oldFiles.sort()
oldFile = oldFiles[-1]
print oldFile
oldResult = np.load(oldFile)

oldResult = np.load('/data/ljo31b/EELs/galsub/emceeruns/darwin/J0837_parametric_DPL_darwin_2_Iband_ctd')
