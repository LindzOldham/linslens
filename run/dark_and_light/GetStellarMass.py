...
def GetStellarMass(oresult,Mstar,r,scale,name,deprojected=False):
    ...
    LM[0] = LM[1]
    light *= Mstar*1e12/norm
    if deprojected:
        return lr,light,LM
    else:
        print 'projected stellar mass'
        sb /= norm
        sb *= Mstar*1e12
        if name == 'J2228':
            sb*=5.
        sr = r[:-100]
        cumsb = sr*0.
        sbmodel = splrep(sr,sb[:-100]*2.*np.pi*sr)
        for i in range(sr.size):
            cumsb[i] = splint(0,sr[i],sbmodel)
        print cumsb[-1]
        cumsb /= cumsb[-1]
        cumsb *= Mstar*1e12
        return lr, sb[:-100], cumsb
