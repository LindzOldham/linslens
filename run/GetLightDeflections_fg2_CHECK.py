...
def Deflect(model,kmodel,name,debug=False):
    model.Initialise()
    kmodel.Initialise()
    if debug:
    ...
    if model.galno == 1.:
        ...
        im1 = SBModels.Sersic('lens 1',{'x':gal.x,'y':gal.y,'n':gal.n,'re':gal.re,'q':gal.q,'pa':gal.pa})
        ...
        im1 = SBObjects.Sersic('lens 1',{'x':galk1.x,'y':galk1.y,'n':galk1.n,'re':galk1.re,'q':galk1.q,'pa':galk1.pa,'c':2.})
        im2 = SBObjects.Sersic('lens 2',{'x':galk2.x,'y':galk2.y,'n':galk2.n,'re':galk2.re,'q':galk2.q,'pa':galk2.pa,'c':2.})                  
        im1.setPars()
        im2.setPars()
        ...
        #print lens1.b, lens2.b 
        #print lens1.getMass(sig_crit)
        #print lens2.getMass(sig_crit)
        lenses = [lens1, lens2]
    ...
    sb1 = im1.pixeval(xk,yk,kmodel.pix/kmodel.OVRS)
    sb2 = im2.pixeval(xk,yk,kmodel.pix/kmodel.OVRS)
    psf = kmodel.psf
    sb1 =  kmodel.fits[0]*convolve.convolve(sb1,psf,False)[0]
    sb2 = kmodel.fits[1]*convolve.convolve(sb2,psf,False)[0]
    '''pl.figure()
    pl.imshow(sb1)
    pl.colorbar()
    pl.figure()
    ...
    sb = sb1 + sb2
    '''pl.figure()
    pl.imshow(sb,interpolation='nearest',origin='lower')
    pl.colorbar()
    gal = kmodel.comps[:2].sum(0)
    pl.figure()
    pl.imshow(gal,interpolation='nearest',origin='lower')
    pl.colorbar()
    pl.figure()
    pl.imshow(sb-gal,interpolation='nearest',origin='lower')
    pl.colorbar()
    pl.show()'''
    ...
    for lens in lenses:
        ...
        #print n, fracs[n]
        n += 1
    np.save('test',[xk,yk,x0k,y0k])
    pl.figure()
    pl.scatter(x0k,y0k,s=50,c=xk,edgecolors='none')
    pl.colorbar()
    pl.figure()
    pl.scatter(x0k,y0k,c=yk,edgecolors='none')
    pl.colorbar()
    pl.figure()
    pl.scatter(xk,yk,c=x0k,edgecolors='none')
    pl.colorbar()
    pl.figure()
    pl.scatter(xk,yk,c=y0k,edgecolors='none')
    pl.show()
...
for ii in range(len(names)):
    ...
    print name
    Deflect(model,kmodel,name,debug=False)
