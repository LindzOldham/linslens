...
def ellipse(ax,ay,xbins,ybins,smooth=[0.5,0.5],color='CornflowerBlue'):
...
for ii in range(len(names)):
    ...
    xbins=np.linspace(min(x)-0.1,max(x)+0.5,100)
    ...
    elif name in ['J0913', 'J1347']:
        xbins=np.linspace(min(x)-1.5,max(x)+0.5,120)
        ybins=np.linspace(min(y)-0.1,max(y)+0.1,120)
    elif name in ['J0901','J1144']:
        xbins=np.linspace(min(x)-1.5,max(x)+0.5,150)
        ybins=np.linspace(min(y)-0.1,max(y)+0.1,150)
    elif name == 'J2228':
        xbins=np.linspace(min(x)-1.5,max(x)+0.5,120)
        ybins=np.linspace(min(y)-0.1,max(y)+0.1,120)
    #if name in ['J0913','J1446','J1606','J1347']:
    #    xbins=np.linspace(min(x)-1.5,max(x)+0.5,125)
    #    ybins=np.linspace(min(y)-0.1,max(y)+0.1,125)
    
    sigmas[ii] = ellipse(x,y,xbins,ybins,smooth=[3,2]) # this breaks it!
    pl.title(name)
    print sigmas
#pl.close('all')
#np.save('/data/ljo31/Lens/LensParams/ReMu_covariances.npy',sigmas)
