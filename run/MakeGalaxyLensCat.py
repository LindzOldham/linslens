...
def MakeTab(model):
    ...
    np.save('/data/ljo31/Lens/PDFs/212_lensgal_'+str(model.name), [np.array([model.muPDF]),np.array([model.magPDF]),np.array([ model.fitPDF]), model.viPDF,np.array([model.RePDF])])
    return med,lo,hi
...
tbhdu.writeto('/data/ljo31/Lens/LensParams/Phot_1src_lensgals_new.fits',clobber=True)
...
coldefs = ColDefs([c1,c2,c3,c4,c5,c6,c7,c8,c2l,c3l,c4l,c5l,c6l,c7l,c8l,c2h,c3h,c4h,c5h,c6h,c7h,c8h])
tbhdu = BinTableHDU.from_columns(coldefs)
tbhdu.writeto('/data/ljo31/Lens/LensParams/Phot_2src_lensgals_new.fits',clobber=True)
