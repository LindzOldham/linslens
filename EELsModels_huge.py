...
class EELs:
    ...
    def MakeDict(self):
        ...
        a1,a2 = np.unravel_index(self.lp.argmax(),self.lp.shape)
        l,u,d = [], [], []
        for key in self.dic.keys():
            f = self.dic[key].reshape((self.trace.shape[0]*self.trace.shape[1]))
            lo,med,up = np.percentile(f,50)-np.percentile(f,16), self.dic[key][a1,a2], np.percentile(f,84)-np.percentile(f,50) 
            d.append((key,med))
            l.append((key,lo))
            u.append((key,up))
        ...
        if 'Source 1 x' not in self.dic.keys():
            for key in 'x', 'y':
                ...
                lo,med,up = np.percentile(f,50)-np.percentile(f,16), self.dic['Source 2 '+key][a1,a2], np.percentile(f,84)-np.percentile(f,50) 
        ...
        if 'Galaxy 2 x' not in self.dic.keys():
            for key in 'x', 'y':
                ...
                lo,med,up = np.percentile(f,50)-np.percentile(f,16), self.dic['Galaxy 1 '+key][a1,a2], np.percentile(f,84)-np.percentile(f,50) 
...
###
    ...
    def GetFits(self,plotsep=False,plotresid=False,plotcomps=False,cmap='afmhot'):
        ...
        for i in range(len(self.imgs)):
            ...
            image = self.imgs[i]
            sigma = self.sigs[i]
            #print xo.shape, image.shape,self.PSFs[i].shape
            psf = self.PSFs[i]
            ...
            for src in self.srcs:
                ...
                if 'boxiness' in self.dic.keys():
                    if src.pars['q']<0.2:
                        ...
                        tmp = src.boxypixeval(x0,y0,1./OVRS,csub=31,c=self.Ddic['boxiness'])
                    else:
    ...
    def GetPDFs(self,kpc=False):
        ...
        yc,xc=iT.overSample(self.img1.shape,OVRS)
        colours = [bands[self.name], 'F814W']
        for b1 in range(0,len(self.dictionaries),10):
            srcs,gals,lenses = [],[],[]
            for number in range(1,1+self.srcno):
                name = 'Source '+str(number)
                p = {}
            ...
            if len(self.srcs)==1:
                ...
                if np.any(fits[0][-3:-1]==0):
                    ...
                    mag_v = srcs[ii].getMag(fits[0][ii],self.ZPs[0])
                else:
                ...
                if np.any(fits[1][-3:-1]==0):
                    ...
                    mag_i = srcs[ii].getMag(fits[1][ii],self.ZPs[1])
                else:
    ...
    def EasyAddPDFs(self):
        self.muPDF, self.magPDF, self.RePDF = np.load('/data/ljo31/Lens/PDFs/211_huge__'+str(self.name)+'.npy')
    ...
    def tabforpaper(self):
        if self.srcno==1 or self.name == 'J0837':
            print self.name, '&', '%.4f'%self.z, '&', '%.4f'%lens_redshifts[self.name], '&', '%.2f'%self.mag_v, '&', '%.2f'%self.mag_i, '& -- &', '%.2f'%self.srcs[0].pars['n'], '&', '%.2f'%(0.05*self.srcs[0].pars['re']*self.scale), '&', '%.2f'%self.srcs[0].pars['pa'], '&', '%.2f'%self.srcs[0].pars['q'], '&', '%.2f'%self.Mv, r'\\'
        else:
            print self.name, '&', '%.4f'%self.z, '&', '%.4f'%lens_redshifts[self.name], '&', '%.2f'%self.mag_v, '&', '%.2f'%self.mag_i, '& -- &', '%.2f'%self.srcs[0].pars['n'], '&', '%.2f'%(0.05*self.scale*self.srcs[0].pars['re']), '&', '%.2f'%self.srcs[1].pars['n'], '&', '%.2f'%(0.05*self.scale*self.srcs[1].pars['re']), '&', '%.2f'%(self.scale*self.Re_v), '&', '%.2f'%self.Mv, r'\\'
